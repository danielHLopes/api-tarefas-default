//conectar com login e gerar token
const conexao = require('../config/conexao')
const jwt = require('jsonwebtoken')//gera o token
const bcrypt = require('bcrypt')//decripta
        ...
        if (err){
            ...
            bcrypt.compare(senha, rows[0].senha, (err, resp) =>{
                if(resp){
                    ...
                    jwt.sign({usuario}, process.env.SECRET, {expiresIn: 30000000}, (err, token)=>{
                        res.status(200)
                        res.json({"auth": true, "token": token})
                    })
                }else{
                    res.status(404)
                    res.json({"auth": false,"message" : "E-mail ou senha invalidos"})
                }
            })
        }else{
            res.status(404)
